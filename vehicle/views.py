...
def mechanic_dashboard_view2(request):
    ...
    work_in_progress=models.Request.objects.all().filter(mechanic_id=mechanic2.id,status='Repairing').count()
    work_completed=models.Request.objects.all().filter(mechanic_id=mechanic2.id,status='Repairing Done').count()
    new_work_assigned=models.Request.objects.all().filter(mechanic_id=mechanic2.id,status='Approved').count()
    dict={
    'work_in_progress':work_in_progress,
    'work_completed':work_completed,
    'new_work_assigned':new_work_assigned,
...
def mechanic_work_assigned_view2(request):
    ...
    return render(request,'vehicle/mechanic2_work_assigned.html',{'works':works,'mechanic2':mechanic2})
